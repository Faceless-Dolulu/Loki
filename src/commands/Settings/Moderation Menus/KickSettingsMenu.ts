...
export async function kickSettingsMenu<T extends MessageComponentInteraction>(
	interaction: T
): Promise<void> {
	...
	const logChannelSelect = new ChannelSelectMenuBuilder()
		...
		.setCustomId(`SelectKickLogChannel`)
		.setPlaceholder(`Log Channel`)
		.setMaxValues(1)
		.setMinValues(0);
	if (config.kick.logChannel) {
		logChannelSelect.setDefaultChannels(config.kick.logChannel);
	}
	...
	const whitelistedRolesSelect = new RoleSelectMenuBuilder()
		...
		.setMinValues(0);
	if (config.kick.whitelistedRoles) {
		whitelistedRolesSelect.setDefaultRoles(config.kick.whitelistedRoles);
	}
	const logChannelRow =
	...
	const settings = new KickSettings(
		config.kick.enabled,
		config.kick.reasonRequired,
		config.kick.evidenceRequired,
		config.kick.whitelistedRoles as string[],
		config.kick.logChannel as string | null
	);
	...
	collector.on(`collect`, async (i) => {
		async function updateMenu(finalized: boolean) {
			const hasChanges = settings.hasChangedFrom(config.kick);
			if (hasChanges === true) {
