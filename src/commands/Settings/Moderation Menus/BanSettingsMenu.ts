...
export async function banSettingsMenu(
	...
	const logChannelSelect = new ChannelSelectMenuBuilder()
		...
		.setCustomId(`SelectBanLogChannel`)
		.setPlaceholder(`Log Channel`)
		.setMaxValues(1)
		.setMinValues(0);
	if (config.ban.logChannel) {
		logChannelSelect.setDefaultChannels(config.ban.logChannel);
	}
	...
	const whitelistedRolesSelect = new RoleSelectMenuBuilder()
		...
		.setMinValues(0);
	if (config.ban.whitelistedRoles) {
		whitelistedRolesSelect.setDefaultRoles(config.ban.whitelistedRoles);
	}
	const logChannelRow =
	...
	const settings = new BanSettings(
		config.ban.enabled,
		config.ban.reasonRequired,
		config.ban.evidenceRequired,
		config.ban.whitelistedRoles as string[],
		config.ban.logChannel as string | null
	);
	...
	collector.on(`collect`, async (i) => {
		async function updateMenu(finalized: boolean) {
			const hasChanges = settings.hasChangedFrom(config.ban);
			if (hasChanges === true) {
