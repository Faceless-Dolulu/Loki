...
export const data = new SlashCommandBuilder()
    ...
    .setName("admin-only")
    .setDescription("Sets whether the ticket is only viewable to server Admins")
    .setRequired(true)))
    .addSubcommand((command) => command
    .setName("close")
    ...
    .addUserOption((option) => option
    .setName("user")
    .setDescription(`The user you want to remove from this ticket discussion`)
    .setRequired(true)));
// .addSubcommand(command => command
// 	.setName('rename')
...
export async function run({ interaction, client, handler }) {
    try {
        ...
        if (command === "open") {
            ...
            const openingMessage = serverConfig?.ticketOpeningMessage ??
            ...
            const category = interaction.guild?.channels.cache.get(serverConfig?.ticketCategoryId) ??
                (await interaction.guild?.channels.fetch(serverConfig?.ticketCategoryId));
            if (!category) {
        ...
        if (command == "close") {
            ...
            if (!serverConfig?.ticketCategoryId ||
            ...
            if (interaction.user.id !== ticket?.ticketUserId)
                userQualified = false;
            if (userRoles.cache.hasAny(...adminRoles) ||
            ...
            if (userQualified) {
                ...
                const archiveChannel = interaction.guild?.channels.cache.get(serverConfig?.ticketArchiveChannelId) ??
                    (await interaction.guild?.channels.fetch(serverConfig?.ticketArchiveChannelId));
                const adminArchiveChannel = interaction.guild?.channels.cache.get(serverConfig?.ticketAdminArchiveChannelId) ??
                    (await interaction.guild?.channels.fetch(serverConfig?.ticketAdminArchiveChannelId));
                ...
                await interaction.editReply(`Generating transcript... Can take a few minutes if many messages were sent...`);
                const transcript = await discordTranscripts.createTranscript(channel, {
                    filename: `${channel.name} - ${yyyy}-${mm}-${dd}.html`,
                    hydrate: true,
                    returnType: ExportReturnType.Attachment,
                });
                await interaction.editReply(`Transcript generated... Archiving transcript...`);
                if (ticket?.adminOnly === false) {
                    ...
                    await interaction
                        .editReply(`Transcript archived in ${archiveChannel}. This channel will be deleted shortly...`)
                        .then(() => {
                        setTimeout(() => {
                            channel.delete();
                        }, 5000);
                ...
                if (ticket?.adminOnly === true) {
                    ...
                    await interaction
                        .editReply(`Transcript archived in ${adminArchiveChannel}. This channel will be deleted shortly...`)
                        .then(() => {
                        setTimeout(() => {
                            channel.delete();
                        }, 5000);
        ...
        if (command == "add-user") {
            ...
            if (userRoles.cache.hasAny(...adminRoles) ||
                ...
                userQualified = true;
            if (!userQualified) {
                interaction.reply(`❌ You do not have the requirements needed to add a user to this channel`);
                return;
            }
            if (userQualified) {
                await channel.permissionOverwrites
                    ...
                    console.log(`An error occurred in ${fileURLToPath(import.meta.url)}:\n`, error);
                });
                interaction.reply({
        ...
        if (command == "remove-user") {
            ...
            if (interaction.user.id !== ticket?.ticketUserId)
                userQualified = false;
            if (userRoles.cache.hasAny(...adminRoles, ...moderatorRoles) ||
                interaction.user.id === ticket?.ticketUserId)
                userQualified = true;
            if (!userQualified) {
                interaction.reply({
            ...
            if (userQualified) {
                if (targetMember.roles.cache.hasAny(...adminRoles, ...moderatorRoles) ||
                ...
                interaction.reply(`✅ ${targetMember} has been removed from this ticket!`);
                return;
            }
        }
    }
