...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option
    .setName("duration")
    .setDescription(`The duration of the user's ban. Default is permanent. Ex. 30d, 12h, 8w, 10y`))
    .addStringOption((option) => option
    .setName("delete-messages")
    .setDescription("The amount of time to delete the members messages from. Max is 7 days."))
...
export async function run({ interaction, client, handler }) {
    ...
    try {
        ...
        const banLogChannel = interaction.guild?.channels.cache.get(serverConfig?.banLogsChannelId) ??
            (await interaction.guild?.channels.fetch(serverConfig?.banLogsChannelId));
        ...
        await interaction.guild?.bans
            ...
            if (time === "0") {
                ...
                targetUser
                    ...
                    .catch(() => { });
                await interaction.followUp(`:hammer: ${targetUser} has been banned indefinitely`);
            }
            if (time !== "0") {
                ...
                targetUser
                    ...
                    embeds: [
                        new EmbedBuilder()
                            .setColor(0xff0000)
                            .setDescription(`⚠ You were **banned** from ${interaction.guild?.name}\n📄 **Reason:** ${reason}\n⏱ **Duration:** ${prettyMilliseconds(duration, { verbose: true })}`)
                            .setThumbnail(interaction.guild?.iconURL()),
                    ],
                })
                    .catch(() => { });
            }
            await interaction.followUp(`:hammer: ${targetUser} has been banned for ${prettyMilliseconds(duration, { verbose: true })}`);
        });
    }
    catch (error) {
